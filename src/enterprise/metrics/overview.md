...
There are **three metric sources**, and they share one brain — the same discovery, the same
...
- **[SigNoz](./signoz.md)** — point Versus at SigNoz (Cloud or self-hosted) and it discovers
  your services from recent spans, then probes a metric catalog against each one.

All three are **Enterprise**. Pick the source that matches where your metrics live (or run
several); the pages above cover the setup, options, and cost controls for each.
