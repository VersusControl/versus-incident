...
func TestRelatedLogsTimeRangeSchema(t *testing.T) {
	properties := (RelatedLogs{}).ArgsSchema()["properties"].(map[string]any)
	for _, key := range []string{"time_range_minutes", "window_minutes", "start", "end"} {
...
func TestRelatedLogs_WindowAndLimitClamp(t *testing.T) {
	...
	res, err := tool.Invoke(context.Background(), mustArgs(t, relatedLogsArgs{
		TimeRangeArgs: aitools.TimeRangeArgs{WindowMinutes: 999999},
		Limit:         9999,
	}))
	if err != nil {
		t.Fatalf("Invoke: %v", err)
	}
	if got := res.Data["time_range_minutes"]; got != relatedLogsMaxWindow {
		t.Errorf("time_range_minutes = %v, want %d", got, relatedLogsMaxWindow)
	}
	...
	if res.Data["logs_total"] != 300 || res.Data["truncated"] != true {
		t.Errorf("truncation metadata = total:%v truncated:%v", res.Data["logs_total"], res.Data["truncated"])
	}
	if !res.Found {
...
func TestRelatedLogs_OutsideWindowDropped(t *testing.T) {
	...
	tool := RelatedLogs{Reader: r}
	res, err := tool.Invoke(context.Background(), mustArgs(t, relatedLogsArgs{TimeRangeArgs: aitools.TimeRangeArgs{WindowMinutes: 10}}))
	if err != nil {
		t.Fatalf("Invoke: %v", err)
	}
	logs := res.Data["logs"].([]relatedLogLine)
