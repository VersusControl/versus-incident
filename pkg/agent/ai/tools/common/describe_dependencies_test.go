...
func TestDescribeDependencies_WindowClampAndNilStore(t *testing.T) {
	...
	res, err := tool.Invoke(context.Background(), mustArgs(t, describeDependenciesArgs{Service: "api", TimeRangeArgs: aitools.TimeRangeArgs{WindowMinutes: 99999}}))
	if err != nil {
		t.Fatalf("Invoke: %v", err)
	}
	if res.Data["time_range_minutes"] != 1440 {
		t.Errorf("time_range_minutes = %v, want 1440 (clamped)", res.Data["time_range_minutes"])
	}
	up := res.Data["upstream"].([]dependencyNeighbour)
