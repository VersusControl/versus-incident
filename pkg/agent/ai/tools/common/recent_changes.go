...
func (RecentChanges) ArgsSchema() map[string]any {
	return map[string]any{
		...
		}, recentChangesDefaultWindow, recentChangesMaxWindow),
	}
}
...
func (rc RecentChanges) Invoke(ctx context.Context, args json.RawMessage) (*core.ToolResult, error) {
	...
	if rc.Feed == nil {
		return core.UnavailableToolResult(RecentChanges{}.Name(), "change integration not configured"), nil
	}

	records, err := rc.Feed.Changes(ctx, timeRange.Start)
	if err != nil {
	...
	for _, r := range records {
		if r.Timestamp.Before(timeRange.Start) || r.Timestamp.After(timeRange.End) {
			continue
		}
	...
	if len(out) == 0 {
		return rc.miss(a, timeRange), nil
	}
...
// miss is the uniform envelope for a time range or service with no matches.
func (RecentChanges) miss(a recentChangesArgs, timeRange aitools.TimeRange) *core.ToolResult {
	return &core.ToolResult{
		Tool:  RecentChanges{}.Name(),
		Found: false,
		Data: map[string]any{
