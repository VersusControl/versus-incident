...
func TestFindRunbook_LimitClamp(t *testing.T) {
	...
	if idx.gotLimit != findRunbookMaxLimit+1 {
		t.Errorf("limit = %d, want over-fetch %d", idx.gotLimit, findRunbookMaxLimit+1)
	}

	if _, err := tool.Invoke(context.Background(), mustArgs(t, findRunbookArgs{Query: "q", Limit: 0})); err != nil {
		t.Fatalf("Invoke: %v", err)
	}
	if idx.gotLimit != findRunbookDefaultLimit+1 {
		t.Errorf("limit = %d, want default over-fetch %d", idx.gotLimit, findRunbookDefaultLimit+1)
	}
}
...
func TestFindRunbook_RequiresDeps(t *testing.T) {
	...
	result, err := tool.Invoke(context.Background(), mustArgs(t, findRunbookArgs{Query: "q"}))
	assertUnavailable(t, result, err)
}

func TestFindRunbookReportsTruncation(t *testing.T) {
	searcher := &fakeSearcher{matches: []RunbookMatch{{ID: "one"}, {ID: "two"}, {ID: "three"}}}
	result, err := (FindRunbook{Embedder: &recordingEmbedder{}, Index: searcher}).Invoke(context.Background(), mustArgs(t, findRunbookArgs{Query: "q", Limit: 2}))
	if err != nil {
		t.Fatalf("Invoke: %v", err)
	}
	if len(result.Data["matches"].([]RunbookMatch)) != 2 || result.Data["truncated"] != true {
		t.Fatalf("data = %+v", result.Data)
	}
	if _, ok := result.Data["matches_total"]; ok {
		t.Fatal("matches_total must be omitted when the backend cannot provide an exact total")
	}
}
