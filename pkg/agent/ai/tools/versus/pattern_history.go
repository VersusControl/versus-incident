...
func (p PatternHistory) Invoke(_ context.Context, args json.RawMessage) (*core.ToolResult, error) {
	if p.Catalog == nil {
		return core.UnavailableToolResult(PatternHistory{}.Name(), "Versus catalog not configured"), nil
	}
	...
	if s := latestSamples(pat.Samples, patternHistorySampleLimit); len(s) > 0 {
		data["samples"] = s
	}
