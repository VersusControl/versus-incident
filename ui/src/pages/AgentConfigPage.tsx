...
// config panel, secrets (AI API key, source credentials) are reduced to
// a simple "configured / not set" pill. Exported as a panel so
// SettingsPage can compose it as the Agent tab.
      ...
      <div className="card">
        ...
        <div className="card-body">
          <Grid>
            ...
            />
            <KV k="Sources file" v={cfg.data.sources_path || "(inline)"} />
          </Grid>
        </div>
      </div>
        ...
        <div className="card-body">
          {cfg.data.sources.length === 0 ? (
            <div className="text-xs text-ink-400">
              No sources defined. See <code>agent.sources_path</code> or the
              inline <code>agent.sources</code> list.
            </div>
          ) : (
      ...
      <div className="card">
        ...
        <div className="card-body">
          <Grid>
            ...
            />
            <KV k="Temperature" v={String(cfg.data.ai.temperature)} />
          ...
          {cfg.data.service_patterns.length === 0 ? (
            ...
            <ul className="space-y-1">
              {cfg.data.service_patterns.map((p, i) => (
                <li
                  key={i}
                  className="rounded bg-ink-700 px-2 py-1 font-mono text-xs text-ink-200"
                >
                  {p}
                </li>
              ))}
