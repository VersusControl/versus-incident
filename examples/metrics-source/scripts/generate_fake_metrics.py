...
SELF-CONTAINED COPY — this is a verbatim copy of the OSS host-run generator
`versus-incident/scripts/generate_fake_metrics.py`. The enterprise example
ships its own copy (rather than cross-mounting the OSS scripts/ tree) so it
stands alone, exactly as the OSS metrics example expects; keep the two in
sync. The fake-data UX is identical everywhere: same flags, same metric
names the enterprise X10 `prometheus` data source range-queries.
